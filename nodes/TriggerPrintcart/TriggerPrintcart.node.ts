...
export class TriggerPrintcart extends Node {
	authPropertyName = 'authentication';
	description: INodeTypeDescription = {
		displayName: 'TriggerPrintcart',
		icon: 'file:webhookReceiver.svg',
		name: 'TriggerPrintcart',
		group: ['trigger'],
		version: [1, 1.1],
	...
	async webhook(context: IWebhookFunctions): Promise<IWebhookResponseData> {
		const option1 = context.getNodeParameter('option1', {}) as {
			binaryData: boolean;
			ignoreBots: boolean;
			rawBody: boolean;
			responseData?: string;
		...
		if (nodeVersion > 1 && !req.body && !option1.rawBody) {
			try {
				return await this.handleBinaryData(context);
			} catch (error) {}
		}

		if (option1.rawBody && !req.rawBody) {
		...
		return {
			webhookResponse: option1.responseData,
